$(function() {
	...
	function translateToQuneeElements(json, graph) {
		...
		if (json.nodes) {
			Q.forEach(json.nodes, function(data) {
				var node = graph
						.createNode(data.name, data.x || 0, data.y || 0);
				node.set("data", data);
				map[data.id] = node;
			});
		}
	...
	//request("/mybatis/test/getQuneeData", "", onDataCollected);
	$.ajax({
		url:"/mybatis/test/getQuneeData",
		type : "POST",
		dataType : "json",
		success : function(data){
			//var json = JSON.parse(data);
			translateToQuneeElements(data, graph);
		}
	})
});
